...
import Services from "../services/gita.services";
...
class ControllersData {
  ...
  add = async (req: Request, res: Response<ICommonController>, next: NextFunction) => {
    try {
      let validatedData: Validation = await utility.validateAndConvert(AddGitaViewModel, req.body);
      if (validatedData.error) {
        ...
        let validated_data: AddGitaViewModel = validatedData.data as AddGitaViewModel;
